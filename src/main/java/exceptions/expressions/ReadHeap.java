...
public class ReadHeap implements Expression{
    ...
    public Value eval(IDictionary<String, Value> table, IHeap<Value> heap) throws InterpreterException {
        ...
        if (!(addressComponent.getType() instanceof ReferenceType)) throw new InterpreterException("ERROR: The exceptions.expressions does not result in an reference type.");
